...
def train_evaluate_tensorflow (dataset_uri, output_uri, batch_size=100, lr=.001, epochs=100 ):
    ...
    ytest_df = test_df['HeartDisease']
    xtest_df = test_df.drop(['split','HeartDisease'], axis=1)
    ...
    model.add(Dense(units=xtrain_df.shape[1], activation="relu", name=f"linear"))
    ...
    print("\n\n")
    print("\ntrain:\n")
    temp = model.evaluate(xtrain_df.values, ytrain_df)
    print("\nval:\n")
    temp = model.evaluate(xval_df.values, yval_df)
    print("\ntest:\n")
    temp = model.evaluate(xtest_df.values, ytest_df)
